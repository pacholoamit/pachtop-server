...
pub const BASE: Origin<'static> = uri!("/releases");

#[derive(Debug)]
pub struct TauriGHRelease {
    // struct name is whatever the function encompasses
...
const PACHTOP_REPO: &str = "pacholoamit/pachtop";
pub type PachtopRelease = RwLock<TauriGHRelease>;
...
async fn get_latest_release_ttl(
    ...
    guard.expiry = SystemTime::now()
        .checked_add(Duration::from_secs(TTL))
        .unwrap();
    release
}

// /releases/pachtop/win64/1.18.0?msg=""
#[get("/pachtop/<_platform>/<current_version>?<msg>")] // &<other_stuff>
pub async fn pachtop_desktop_api(
    _platform: &str,
    current_version: &str,
    msg: Option<&str>,
    client: &State<Client>,
    cache: &State<PachtopRelease>,
) -> Result<Value, Status> {
    // Status::NoContent
